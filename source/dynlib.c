...
int AAssetManager_open_hook(void *mgr, const char *fname, int mode) {
	char full_fname[256];
	sprintf(full_fname, "ux0:data/enbornx/assets/%s", fname);
...
DIR *AAssetManager_openDir_hook(void *mgr, const char *fname) {
	char full_fname[256];
	sprintf(full_fname, "ux0:data/enbornx/assets/%s", fname);
	return opendir(full_fname);
}

void AAssetDir_close_hook(DIR *assetDir) {
	closedir(assetDir);
}

const char *AAssetDir_getNextFileName_hook(DIR *assetDir) {
	struct dirent *ent;
...
void _pthread_cleanup_push(void * a, void * b) {}
...
so_default_dynlib default_dynlib[] = {
        ...
        { "__pthread_cleanup_pop", (uintptr_t)&_pthread_cleanup_pop },
        { "__pthread_cleanup_push", (uintptr_t)&_pthread_cleanup_push },
        { "pread", (uintptr_t)&pread },
        { "pwrite", (uintptr_t)&pwrite },
        ...
        // ANative
        { "AAssetDir_close", (uintptr_t)&AAssetDir_close_hook },
        { "AAssetDir_getNextFileName", (uintptr_t)&AAssetDir_getNextFileName_hook },
        { "AAssetManager_open", (uintptr_t)&AAssetManager_open_hook },
        { "AAssetManager_openDir", (uintptr_t)&AAssetManager_openDir_hook },
		...
		{ "AConfiguration_getNavHidden", (uintptr_t)&ret0 },
		...
		{ "ANativeActivity_setWindowFormat", (uintptr_t)&ret0 },
		{ "ANativeWindow_getFormat", (uintptr_t)&ret0 },
