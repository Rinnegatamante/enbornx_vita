...
FILE *fopen_soloader(char *fname, char *mode) {
	...
	if (strncmp(fname, "ux0:", 4)) {
		char real_fname[256];
		sprintf(real_fname, "ux0:data/enbornx/assets/%s", fname);
    #ifdef USE_SCELIBC_IO
        ret = sceLibcBridge_fopen(real_fname, mode);
    #else
        ret = fopen(real_fname, mode);
    #endif		
	} else {
    #ifdef USE_SCELIBC_IO
        ret = sceLibcBridge_fopen(fname, mode);
    #else
...
int fstat_soloader(int fd, void *statbuf) {
    ...
    logv_debug("[io] fstat(fd#%i): %i %u", fd, res, st.st_size);
    return res;
}
